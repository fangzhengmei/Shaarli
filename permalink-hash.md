...
| CRC32 自然碰撞（7.7 万条） | 书签量 ≥ 77,000 | 概率 49.8533%（接近 50%） | 几乎必定有碰撞，多条书签 permalink 串扰 | 中（但个人很少攒到这么多） |
| CRC32 自然碰撞（10 万条） | 书签量 ≥ 100,000 | 概率 68.7809% | 同上 | 中高 |
| 人为构造碰撞 | 攻击者故意构造 CRC32 碰撞的 URL | CRC32 可被秒级构造 | 书签被"覆盖"，无法通过短链访问 | 中（但 Shaarli 是自用工具） |
...
| Hash 算法 | CRC32 + base64url（6 字符） | 极快、输出短、URL 安全 | 非加密安全，可被伪造；**有效输出空间仅 2³² ≈ 43 亿**，不是 64⁶ |
